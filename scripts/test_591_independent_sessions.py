...
OUTPUT = Path("docs/data/591-independent-session-test.json")
...
def warm_session(page, road, bootstrap_url, logs):
    found = []
    ...
    for round_no in (1, 2):
        try:
            ...
            if round_no == 2:
                target += ("&" if "?" in target else "?") + f"_r={int(time.time())}"
...
def test_one_road(browser, road, street_id):
    logs = []
    ...
    try:
        ...
        template = warm_session(page, road, bootstrap_url, logs)
        if not template:
            return {"road": road, "streetid": street_id, "success": False, "count": 0, "pages": 0, "logs": logs}

        for page_no in range(1, 11):
            api_url = build_api_url(template, street_id, (page_no - 1) * 30, page_no)
            payload, ok = request_json(context, api_url, logs, f"第 {page_no} 頁")
            if not ok:
                return {"road": road, "streetid": street_id, "success": False, "count": count, "pages": pages, "logs": logs}
        ...
        return {"road": road, "streetid": street_id, "success": True, "count": count, "pages": pages, "logs": logs}
    finally:
        context.close()
...
def main():
    ...
    with sync_playwright() as p:
        ...
        for index, (road, street_id) in enumerate(core.WATCH_591_STREETS.items(), start=1):
            result = test_one_road(browser, road, street_id)
            results.append(result)
            print(f"[{index}/7] {road}: success={result['success']} count={result['count']} pages={result['pages']}")
    ...
    report = {
        "mode": "independent_browser_context_per_road",
        "successCount": success_count,
        "totalRoads": len(results),
        "allSucceeded": success_count == len(results),
        "results": results,
