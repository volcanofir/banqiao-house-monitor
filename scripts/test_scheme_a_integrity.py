...
def test_transitive_591_floor_bridge():
    # Exact area/price makes all adjacent pairs eligible on fingerprint alone.
    # The cluster-level guard must still stop 2F -> unknown -> 3F transitivity.
    rows = [
        row("591", "591:A", "同社區二樓"),
        row("591", "591:B", "同社區美寓"),
        row("591", "591:C", "同社區三樓"),
    ...
    return {
        ...
        "blocked": v4.REGROUP_STATS.get("clusterFloorConflictBlocked"),
        "clusterFloors": [g.get("preview591ClusterFloors") for g in groups],
    }


def test_sinyi_unknown_floor_multi_attach_guard():
    # One floor-unknown Sinyi listing must not become a bridge for two explicit floors.
    external = [
        row("信義房屋", "信義房屋:S1", "同社區方正美寓"),
        row("591", "591:D", "同社區方正二樓"),
        row("591", "591:E", "同社區方正三樓"),
    ]
    v4.pair_591_info = v5.pair_591_info
    ...
    return {
        "groupCount": len(groups),
        "sinyiAttached591": len(attached_591),
        "blocked": v4.GROUP_INTEGRITY_STATS.get("crossSourceMultiAttachFloorConflictBlocked"),
...
def test_structured_company_floor_wins():
    company = {
        ...
        "title": "測試三樓",
        "address": "新北市板橋區中山路二段",
        "text": "測試物件 建坪32.33 主32.333/3樓 3房2廳",
        "floor": "3/3樓",
    }
    parsed = v6.structured_listing_floors(company, company=True)
    assert parsed == {3}, parsed
    assert 33 not in parsed, parsed
    raw_helper = v4.title_floor_tokens("主32.333/3樓")
    assert raw_helper == {3}, raw_helper
    return {"structuredFloors": sorted(parsed), "domHelperFloors": sorted(raw_helper)}
